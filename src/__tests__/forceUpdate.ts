import { describe, expect, test, vi } from 'vitest';
import { memoize } from '../index.js';

// Have `vitest` ignore any rejection it sees.
process.on('unhandledRejection', () => undefined);

interface Type {
  number: number;
}

const method = (one: number, two: Type) => one + two.number;
const promiseMethodResolves = (one: number, two: Type) =>
  new Promise((resolve) =>
    setTimeout(() => {
      resolve(one + two.number);
    }, 100),
  );
const promiseMethodRejects =
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  (one: number, two: Type) =>
    new Promise((resolve, reject) =>
      setTimeout(() => {
        reject(new Error('boom'));
      }, 100),
    );

describe('success', () => {
  test('will refresh the cache', () => {
    const memoized = memoize(method, {
      forceUpdate: (args) => args[1].number % 2 === 0,
      maxSize: 2,
    });

    const mutated = { number: 5 };

    const result = memoized(6, mutated);

    expect(result).toBe(11);

    mutated.number = 11;

    const mutatedResult = memoized(6, mutated);

    // Result was not recalculated because `forceUpdate` returned `false` and the values are
    // seen as unchanged.
    expect(mutatedResult).toBe(result);

    mutated.number = 10;

    const refreshedResult = memoized(6, mutated);

    // Result was recalculated because `forceUpdate` returned `true`.
    expect(refreshedResult).not.toBe(result);
    expect(refreshedResult).toBe(16);

    const { keys, values } = memoized.cache.snapshot;

    expect(keys).toEqual([[6, mutated]]);
    expect(values).toEqual([16]);
  });

  test('will refresh the cache based on external values', async () => {
    const mockMethod = vi.fn(method);

    let lastUpdate = Date.now();

    const memoized = memoize(mockMethod, {
      forceUpdate() {
        const now = Date.now();
        const last = lastUpdate;

        lastUpdate = now;

        return last + 100 < now;
      },
      maxSize: 2,
    });

    const mutated = { number: 5 };

    memoized(6, mutated);
    memoized(6, mutated);
    memoized(6, mutated);

    expect(mockMethod).toHaveBeenCalledTimes(1);

    await new Promise((resolve) => setTimeout(resolve, 200));

    memoized(6, mutated);

    expect(mockMethod).toHaveBeenCalledTimes(2);
  });

  test('will refresh the cache when used with promises', async () => {
    const memoized = memoize(promiseMethodResolves, {
      async: true,
      forceUpdate: (args) => args[1].number % 2 === 0,
      maxSize: 2,
    });

    const mutated = { number: 5 };

    const result = await memoized(6, mutated);

    expect(result).toBe(11);

    mutated.number = 11;

    const mutatedResult = await memoized(6, mutated);

    // Result was not recalculated because `forceUpdate` returned `false` and the values are
    // seen as unchanged.
    expect(mutatedResult).toBe(result);

    mutated.number = 10;

    const refreshedResult = await memoized(6, mutated);

    // Result was recalculated because `forceUpdate` returned `true`.
    expect(refreshedResult).not.toBe(result);
    expect(refreshedResult).toBe(16);

    const cachedItem = memoized.cache.get([6, mutated]);

    expect(cachedItem).toEqual(Promise.resolve(16));
  });

  test.fails(
    'will refresh the cache when used with custom key transformers',
    () => {
      interface ConditionalIncrement {
        force?: boolean;
      }

      let count = 0;

      const increment = (_?: ConditionalIncrement) => ++count;

      const memoized = memoize(increment, {
        forceUpdate: (args) => args[0]?.force === true,
        maxSize: 2,
        serialize: () => ['always same'],
      });

      expect(memoized()).toBe(1);
      expect(memoized()).toBe(1);
      expect(memoized({ force: true })).toBe(2);
      expect(memoized()).toBe(2);
    },
  );
});

describe('fail', () => {
  test('surfaces the error if the function fails', () => {
    const memoized = memoize(
      (_1: number, _2: Type) => {
        throw new Error('boom');
      },
      { forceUpdate: (args) => args[1].number % 2 === 0, maxSize: 2 },
    );

    expect(() => memoized(6, { number: 5 })).toThrow(new Error('boom'));
  });

  test('surfaces the error if the promise rejects', async () => {
    const memoized = memoize(promiseMethodRejects, {
      async: true,
      forceUpdate: (args) => args[1].number % 2 === 0,
      maxSize: 2,
    });

    await expect(memoized(6, { number: 5 })).rejects.toEqual(new Error('boom'));
  });

  test('should have nothing in cache if promise is rejected and key was never present', async () => {
    const memoized = memoize(promiseMethodRejects, {
      async: true,
      forceUpdate: (args) => args[1].number % 2 === 0,
      maxSize: 2,
    });

    await expect(memoized(6, { number: 5 })).rejects.toEqual(new Error('boom'));

    expect(memoized.cache.snapshot.keys).toEqual([]);
    expect(memoized.cache.snapshot.values).toEqual([]);
  });

  test('should have nothing in cache if promise is rejected and key was present', async () => {
    const memoized = memoize(promiseMethodRejects, {
      async: true,
      forceUpdate: (args) => args[1].number % 2 === 0,
      maxSize: 2,
    });

    const mutated = { number: 5 };
    const error = new Error('boom');

    try {
      const rejection = Promise.reject(error);

      memoized.cache.set([6, mutated], rejection);

      expect(memoized.cache.get([6, mutated])).toEqual(rejection);

      await rejection;
    } catch (e) {
      expect(e).toBe(error);
    }

    expect(memoized.cache.snapshot.keys).toEqual([]);
    expect(memoized.cache.snapshot.values).toEqual([]);
  });
});

describe('infrastructure', () => {
  test('should have all the static properties of a standard memoized method', () => {
    const memoized = memoize(promiseMethodResolves, {
      forceUpdate: (args) => args[1].number % 2 === 0,
      maxSize: 2,
    });
    const standardMoized = memoize(promiseMethodResolves, { maxSize: 2 });

    expect(Object.getOwnPropertyNames(memoized)).toEqual(
      Object.getOwnPropertyNames(standardMoized),
    );
  });
});
